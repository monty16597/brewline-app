...
locals {
  ...
  # The fast-checkout experiment: route payments through the processor's slower settlement tier.
  # Checkout timeout must accommodate the payment gateway's 5-second delay plus network overhead,
  # so it is set to 30 seconds to prevent timeout errors when the payment gateway is slow.
  checkout_timeout = 30
